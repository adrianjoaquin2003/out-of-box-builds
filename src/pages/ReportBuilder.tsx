...
export default function ReportBuilder() {
  ...
  const fetchSession = async () => {
    try {
      ...
      let metrics = Array.isArray(data.available_metrics) ? (data.available_metrics as unknown as Metric[]) : [];
  ...
  const fetchReport = async () => {
    try {
      const { data, error } = await supabase.from("saved_reports").select("*").eq("id", reportId).single();
  ...
  const removeMetricFromChart = (chartId: string, metric: string) => {
    ...
    ); // Remove chart if no metrics left
  };
                  ...
                  <Card key={metric.key} className="hover:shadow-md transition-shadow">
                    <CardContent className="p-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Button size="sm" variant="ghost" onClick={() => addChart(metric.key)}>
            ...
            {availableMetrics.length === 0 ? (
              ...
              <div className="flex items-center justify-center h-[calc(100vh-200px)]">
                <Card className="max-w-md text-center">
                  ...
                  <CardContent>
                    <p className="text-muted-foreground mb-4">
                      Select metrics from the left sidebar to add charts to your report. You can customize each chart's
      ...
      <Dialog open={showSaveDialog} onOpenChange={setShowSaveDialog}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button onClick={handleSaveReport} disabled={!reportName.trim() || isSaving} className="flex-1">
